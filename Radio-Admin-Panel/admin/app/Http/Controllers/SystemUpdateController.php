...
class SystemUpdateController extends Controller
{
    ...
    public function index() {
        $system_version = Setting::where('type', 'system_version')->first();
        return view('system_update.index', compact('system_version'));
    }
    ...
    public function update(Request $request) {
        ...
        try{
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://validator.wrteam.in/radio_app_validator?purchase_code=' . $request->purchase_code . '&domain_url=' . $app_url . '',
                    ...
                    if (file_exists($ver_file) && file_exists($source_path)) {
                        ...
                        if (rename($ver_file, $ver_file1) && rename($source_path, $source_path1)) {
                            ...
                            if ($current_version['system_version'] == $version_file['current_version']) {
                                ...
                                $response = array(
                                    'error' => true,
                                    'message' => 'System Already Updated'
                                );
                                return response()->json($response);
                            } else {
                                unlink($source_path1);
